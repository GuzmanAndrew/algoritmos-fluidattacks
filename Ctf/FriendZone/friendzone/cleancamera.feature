...
Feature:
  ...
  Scenario: Information gathering
    ...
    Service Info: Hosts: FRIENDZONE, 127.0.0.1; OSs: Unix, Linux;
    CPE: cpe:/o:linux:linux_kernel
    """
    And I can see that it has a service HTTP
    When I decide to run port 80
    Then I see that port "80" has a very simple web page
    And it is because that page only has one image and several paragraphs
    When I download the image to scan it later
    Then I make the decision to run a command
    ...
    """
    Then I put other domain on the host in case it has something interesting
    ...
    """
    When I realize that its version is inferior
  ...
  Scenario: Success: Pass1 in SmbClient
    ...
    """
    When I decide to run another command
    """
  ...
  Scenario: Success: Getting User Flag
    ...
    And that is because with "nmap" I found a "ssl / http" service
    ...
    And it's a complaint about failing because of parameters in the URL
